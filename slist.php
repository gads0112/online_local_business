...
<?php $cat=$_GET['cat']; ?>
	...
	<?php
		$inc=4;
		$query=mysqli_query($con,"select * from product where shp_id = '$cat'");
		while($row=mysqli_fetch_array($query)){
		$qty=$row['item_quatity'];
		$qty=$row['item_quatity'];
			$low_qty=0;
		$item_id =  $row['product_id'];
			...
			?>
				...
				<?php if($qty<=$low_qty)
				{
				 echo "<img style='width:230px; height:230px; padding:auto; margin:auto;' class='thumbnail' src=".$row['item_image'].">" ;
                 echo"<div style='height:60px; width:230px; margin-left:17px;'><strong>".$row['item_name']."</strong></div>";
				 echo"<div style='height:60px; width:230px; margin-left:17px;'><strong><center>Out of Stock</center></strong></div></div></div>";
				}
                else{			
				   ?>
